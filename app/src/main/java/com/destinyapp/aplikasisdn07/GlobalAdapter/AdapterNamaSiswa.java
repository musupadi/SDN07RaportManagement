...
public class AdapterNamaSiswa extends ArrayAdapter<DataModel> {
    private List<DataModel> dmListFull;

    public AdapterNamaSiswa(@NonNull Context context, @NonNull List<DataModel> List) {
    ...
    private Filter countryFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (DataModel item : dmListFull) {
                    if (item.getNama_siswa().toLowerCase().contains(filterPattern)) {
        ...
        public CharSequence convertResultToString(Object resultValue) {
            return ((DataModel) resultValue).getNama_siswa();
        }
    };
}